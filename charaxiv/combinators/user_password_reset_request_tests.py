...
async def test_user_password_reset_request(password_hasher: PasswordHasher) -> None:
    ...
    assert manager.mock_calls == [
        ...
        mock.call.user_password_reset_exists(userid=user.id),
        mock.call.secret_token_generate(),
        mock.call.user_password_reset_create(userid=user.id, token=token),
...
async def test_user_password_reset_request__password_reset_request_exists(password_hasher: PasswordHasher) -> None:
    ...
    assert manager.mock_calls == [
        ...
        mock.call.user_get_by_email(email=user.email),
        mock.call.user_password_reset_exists(userid=user.id),
        mock.call.user_password_reset_delete(userid=user.id),
        mock.call.secret_token_generate(),
        mock.call.user_password_reset_create(userid=user.id, token=token),
        mock.call.user_password_reset_mail_send(email=user.email, token=token),
        mock.call.context_manager.__aexit__(None, None, None),
