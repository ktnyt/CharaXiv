...
async def test_user_authenticate(password_hasher: PasswordHasher) -> None:
    ...
    # Setup mocks
    manager = mock.Mock()
    manager.db_user_get_by_id = mock.AsyncMock(spec=protocols.db_user_get_by_id.Protocol, side_effect=[user])

    # Instantiate combinator
    combinator = Combinator(db_user_get_by_id=manager.db_user_get_by_id)
    ...
    assert manager.mock_calls == [
        mock.call.db_user_get_by_id(id=user_id),
    ]
...
async def test_user_authenticate__user_not_found() -> None:
    ...
    # Setup mocks
    manager = mock.Mock()
    manager.db_user_get_by_id = mock.AsyncMock(spec=protocols.db_user_get_by_id.Protocol, side_effect=[None])
